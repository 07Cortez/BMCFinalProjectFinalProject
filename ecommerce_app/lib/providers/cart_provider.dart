...
class CartProvider with ChangeNotifier {
  ...
  CartProvider() {
    log('CartProvider created.');
  }

  void initializeAuthListener() {
    log('CartProvider auth listener initialized');
    _authSubscription = _auth.authStateChanges().listen((User? user) {
      if (user == null) {
        log('User logged out, clearing cart.');
        _userId = null;
        _items = [];
      } else {
        log('User logged in: ${user.uid}. Fetching cart...');
        _userId = user.uid;
        _fetchCart();
      }
      notifyListeners();
  ...
  Future<void> _fetchCart() async {
    ...
    try {
      ...
      if (doc.exists && doc.data()!['cartItems'] != null) {
        ...
        log('Cart fetched successfully: ${(_items).length} items');
      } else {
        _items = [];
      }
    } catch (e) {
  ...
  Future<void> clearCart() async {
    ...
    if (_userId != null) {
      try {
        ...
        log('Firestore cart cleared.');
      } catch (e) {
        log('Error clearing Firestore cart: $e');
      }
    }
    notifyListeners();
  }
